...
from utils.dataloader_multitask import DataLoaderCreator
...
def TRAIN_Func(epochs, batch_size, model_name, volume_dir, mask_dir, feature_maps, learning_rate=0.001, weight_path = None, log_path = None, weight_init = None):
    ...
    for epoch in range(epochs):
        ...
        for i, (volumes, masks1, masks2) in enumerate(train_dataloader):
            ...
            # Calculate Loss
            loss, SSIM = criterion(outputs, masks2, masks1)
            ...
            masks1  = np.array(masks1.squeeze(0).squeeze(0).cpu().detach().numpy())
            outputs = np.array(outputs.squeeze(0).squeeze(0).cpu().detach().numpy())

            PSNR_score = PSNR(masks1, outputs)
